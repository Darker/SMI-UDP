...
QReadWriteLock Crc32::crc_table_lock(QReadWriteLock::Recursive);
Crc32::Crc32()
{
    /*if(!crc_table_initialized) {
        if(crc_table_lock.tryLockForRead(2000)) {
            if(!crc_table_initialized) {
                crc_table_lock.unlock();
                if(crc_table_lock.tryLockForWrite(2000)) {
                    QTextStream(stdout)<<"{\n    ";
                    quint32 iterator = 0;
                    quint32 crc;
                    ...
                    for (int i = 0; i < 256; i++)
                    {
                        ...
                        QTextStream(stdout)<<crc;
                        iterator++;
                    }
                    QTextStream(stdout)<<"\n}";
                }
                else {
                    throw std::runtime_error("Deadlock when trying to initialize crc table!");
                }
            }
        }
        else {
...
quint32 Crc32::calculate(const QByteArray& buffer)
{
    return calculate(buffer, 0, buffer.size()-1);
}

quint32 Crc32::calculate(const QByteArray& buffer, quint32 start, quint32 end)
{
    quint32 crc = 0xFFFFFFFFUL;
    for(size_t i=qMax(start, (quint32)0), l=qMin(end+1, (quint32)buffer.size()); i<l; ++i) {
        crc = crc_table[(crc ^ buffer[i]) & 0xFF] ^ (crc >> 8);
    }
    return crc;
}
...
quint32 Crc32::readFromByteArray(const QByteArray& buffer, quint32 position)
{
    return ((quint32)((unsigned char)buffer[position  ]))*256*256*256 +
           ((quint32)((unsigned char)buffer[position+1]))*256*256*  1 +
           ((quint32)((unsigned char)buffer[position+2]))*256*  1*  1 +
           ((quint32)((unsigned char)buffer[position+3]));
}
